...
import com.techconative.restel.swagger.RestelOpenApiSpec3Parser;
...
public class RestelOpenApiSpec3ParserTest {
    ...
    public void testTestDefinitionResponseNested() {
        parser = new RestelOpenApiSpec3Parser(getOpenAPI());
        List<TestDefinitions> testDefinitions = parser.createTestDefinitions();
        TestDefinitions definitions = testDefinitions.stream().filter(def -> def.getCaseUniqueName().equals("/pet/{petId}:GET")).collect(Collectors.toList()).get(0);
        Assert.assertEquals(8, ObjectMapperUtils.convertToMap(definitions.getExpectedResponse()).size());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getExpectedResponse()).get("animal").isObject());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getExpectedResponse()).get("category").isObject());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getExpectedResponse()).get("group").isArray());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getExpectedResponse()).get("tags").isArray());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getExpectedResponse()).get("photoUrls").isArray());
    }
    ...
    public void testTestDefinitionRequestNested() {
        parser = new RestelOpenApiSpec3Parser(getOpenAPI());
        List<TestDefinitions> testDefinitions = parser.createTestDefinitions();
        TestDefinitions definitions = testDefinitions.stream().filter(def -> def.getCaseUniqueName().equals("/pet:POST")).collect(Collectors.toList()).get(0);
        Assert.assertEquals(8, ObjectMapperUtils.convertToMap(definitions.getRequestBodyParams()).size());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getRequestBodyParams()).get("animal").isObject());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getRequestBodyParams()).get("category").isObject());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getRequestBodyParams()).get("group").isArray());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getRequestBodyParams()).get("tags").isArray());
        Assert.assertTrue(ObjectMapperUtils.convertToJsonNode(definitions.getRequestBodyParams()).get("photoUrls").isArray());
    }
    ...
    public void testTestDefinitionQuery() {
        ...
        Assert.assertEquals("integer", ObjectMapperUtils.convertToJsonNode(definitions.getRequestQueryParams()).get("limit").asText());
        Assert.assertEquals("integer", ObjectMapperUtils.convertToJsonNode(definitions.getRequestQueryParams()).get("offset").asText());
    }
    ...
    public void testTestDefinitionResponseHeader() {
        parser = new RestelOpenApiSpec3Parser(getOpenAPI());
        List<TestDefinitions> testDefinitions = parser.createTestDefinitions();
        TestDefinitions definitions = testDefinitions.stream().filter(def -> def.getCaseUniqueName().equals("/pet/{petId}:DELETE")).collect(Collectors.toList()).get(0);
        Assert.assertEquals(2, ObjectMapperUtils.convertToMap(definitions.getExpectedHeader()).size());
        Assert.assertTrue( ObjectMapperUtils.convertToJsonNode(definitions.getExpectedHeader()).get("X-RateLimit-Limit").isArray());
        Assert.assertEquals(3, ObjectMapperUtils.convertToJsonNode(definitions.getExpectedHeader()).get("X-RateLimit-Limit").get(0).size());
        Assert.assertEquals("integer", ObjectMapperUtils.convertToJsonNode(definitions.getExpectedHeader()).get("X-RateLimit-Remaining").asText());
    }
