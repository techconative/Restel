...
public class ObjectMapperUtilsTest {
    ...
    public void testConvertToMap() {
        ...
        Assert.assertEquals(node.get("name").asText(), ObjectMapperUtils.convertToMap(node.toPrettyString()).get("name"));
    }
    ...
    public void testConvertToMappedString() {
        ObjectMapper mapper = new ObjectMapper();
        ObjectNode node = mapper.createObjectNode().put("name", "Tom");
        Assert.assertEquals(node.get("name").asText(), ObjectMapperUtils.convertToMappedString(node.toPrettyString()).get("name"));
    }
    ...
    public void testConvertToJsonNode() {
        ObjectMapper mapper = new ObjectMapper();
        ObjectNode node = mapper.createObjectNode().put("name", "Tom");
        Assert.assertEquals(node.get("name"), ObjectMapperUtils.convertToJsonNode(node.toPrettyString()).get("name"));
