...
public class RestelOpenApiSpec2Parser {
        ...
        if (CollectionUtils.isNotEmpty(parser.getSchemes())) {
            for (Scheme scheme : parser.getSchemes()) {
                if (!Objects.isNull(scheme) && StringUtils.isNotBlank(parser.getHost())) {
                    baseConfig.setBaseUrl(StringUtils.isNotBlank(parser.getBasePath()) ?
                            scheme.toValue().concat("://").concat(parser.getHost()).concat(parser.getBasePath()) : scheme.toValue().concat("://").concat(parser.getHost()));
                    break;
                }
            }
        }
            ...
            for (String path : parser.getPaths().keySet()) {
                ...
                testDefinitions.addAll(
                        parser.getPaths().get(path).getOperationMap().entrySet().stream()
                                .map(operationEntry -> createTestDef(path, operationEntry, parameters)).collect(Collectors.toList())
                );
            }
        }
     ...
     */
    private TestDefinitions createTestDef(String path, Map.Entry<HttpMethod, Operation> operationEntry, List<Parameter> parameters) {
        ...
        def.setRequestQueryParams(getParams(parameters.stream().filter(par -> par instanceof QueryParameter).map(par -> (QueryParameter) par).collect(Collectors.toList())).toPrettyString());

        ObjectNode headers = getParams(parameters.stream().filter(par -> par instanceof HeaderParameter).map(par -> (HeaderParameter) par).collect(Collectors.toList()));
        ...
        if (CollectionUtils.isNotEmpty(operationEntry.getValue().getParameters())) {
            parameters.addAll(operationEntry.getValue().getParameters());
            hasLocalBodyParams = operationEntry.getValue().getParameters().stream().filter(par -> par instanceof BodyParameter).count() > 0;
            hasLocalFormParams = operationEntry.getValue().getParameters().stream().filter(par -> par instanceof FormParameter).count() > 0;
        }

        ObjectNode formParams = getParams(parameters.stream().filter(par -> par instanceof FormParameter).map(par -> (FormParameter) par).collect(Collectors.toList()));
        JsonNode bodyParams = getBodyParams(parameters.stream().filter(par -> par instanceof BodyParameter).map(par -> (BodyParameter) par).collect(Collectors.toList()));

        if (hasLocalBodyParams) {
            //if has local body params add them first
            def.setRequestBodyParams(getBodyParams(operationEntry.getValue().getParameters().stream().filter(par -> par instanceof BodyParameter).map(par -> (BodyParameter) par).collect(Collectors.toList())).toPrettyString());
        ...
        def.setExpectedHeader(getResponseHeader(operationEntry.getValue().getResponses()).toPrettyString());
     ...
     * @return return  response headers of status code 200 if present, else default response headers or else first entities response headers.
     */
    ...
    /**
     * @param responses Map of status code to {@link Response}
     * @return return  response model of status code 200 if present, else default response model or else first entities (response model) in the response map.
     */
    ...
    private JsonNode getBodyParams(List<BodyParameter> parameters) {
        if (CollectionUtils.isNotEmpty(parameters)) {
            Optional<Object> data = parameters.stream().filter(p -> !Objects.isNull(p.getSchema())).map(par -> getModel(par.getSchema())).findFirst();
            if (data.isPresent()) {
    ...
    /**
     * Constructs the Json schema from the property. If the property is a primitive type will return only property's type (like string,integer,etc..).
     *
     * @param property {@link Property}
     * @return returns a JsonNode of json schema from property. If the property is primitive type, returns string value of property type.
     */
    private Object getProperty(Property property) {
        if (property instanceof ObjectProperty) {
            ObjectNode n = ObjectMapperUtils.getMapper().createObjectNode();
            ((ObjectProperty) property).getProperties().forEach((key, value) -> n.putPOJO(key, getProperty(value)));
            return n;
            ...
            return ObjectMapperUtils.getMapper().createArrayNode().addPOJO(getProperty(((ArrayProperty) property).getItems()));
        } else if (property instanceof ComposedProperty) {
            return getComposedModelFromList(((ComposedProperty) property).getAllOf().stream().map(this::getProperty).collect(Collectors.toList()));
        } else {
    ...
    private Object getModel(Model model) {
        ...
        if (model instanceof RefModel && validRef(((RefModel) model).get$ref())) {
            ...
            return ObjectMapperUtils.getMapper().createArrayNode().addPOJO(getProperty(((ArrayModel) model).getItems()));
        } else if (model instanceof ComposedModel) {
            Object data = getComposedModelFromList(((ComposedModel) model).getAllOf().stream().map(this::getModel).collect(Collectors.toList()));
