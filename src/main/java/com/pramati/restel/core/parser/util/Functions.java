package com.pramati.restel.core.parser.util;
...
import static com.pramati.restel.utils.Utils.*;
...
public class Functions {
     ...
     */
    public static final UnaryOperator<String> TO_CAMEL_CASE = y -> findAndApplyOnString(y, "_[a-zA-Z]",
            x -> x.replace("_", "").toUpperCase());

    /**
     * Input: 'snake_case' string. Output: 'SnakeCase' pascal case string
     */
    public static final UnaryOperator<String> TO_PASCAL_CASE = x -> {
     ...
     */
    public static final Function<Cell, Map<String, Object>> TO_MAP = x -> ObjectMapperUtils.getMapper().convertValue(x.getStringCellValue(),
     ...
     */
    public static final Function<Cell, List<String>> TO_LIST = x -> {
        if (x.getCellType() == CellType.NUMERIC) {
            return Collections.singletonList(String.valueOf((int) x.getNumericCellValue()));
        }
        return Arrays.asList(emptyForNull(x.getStringCellValue()).split(","));
    };

    /**
     * Input csv, output list of strings.
     */
    public static final Function<Cell, List<Integer>> TO_INT_LIST = x -> Arrays
            .asList(emptyForNull(x.getStringCellValue()).split(",")).stream().map(a -> Integer.valueOf(a.replace(" ", ""))).collect(Collectors.toList());

    /**
     * Input csv, output set of strings.
     */
    public static final Function<Cell, Set<String>> TO_SET = x -> new HashSet<>(Arrays
            .asList(emptyForNull(x.getStringCellValue()).split(",")));

    /**
     * Input csv, output Boolean.
     */
    public static final Function<Cell, Boolean> TO_BOOLEAN = x -> x.getCellType() == CellType.BOOLEAN ? x.getBooleanCellValue() : Boolean.TRUE;
