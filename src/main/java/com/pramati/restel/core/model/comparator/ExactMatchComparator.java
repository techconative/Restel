...
public class ExactMatchComparator implements ResponseComparator {
    ...
    /**
     * * Compares the response body and the expected output.
     * * 1. Equals -> If the the order of the key value pair is altered.
     * * 2. Equals -> If the response is a nexted structure.
     * * 3. Not Equal -> For case sensitivity.
     * * 4. Not Equal -> For the order in the nest list is altered.
     * * Eg : actual::{'names":['Sam','Tom']} , expected :: {'names': ['Tom','Sam']}.
     * * 5. Not Equal -> If Object Type is different .
     * * Ex : actual:: {'amount': 5.0} ,expected ::{'amount':5} .
     * * In here, one is decimal and another is int type.
     * *
     *
     * @param restResponse
    ...
    private void evalJson(Object restResponse, Object expectedOutput) {
        ...
        Allure.step("Evaluating with ExactMatcher:: Actual :- " + actualOutputNode.toPrettyString() + " Expected :- " + expectedOutputnode.toPrettyString());
    ...
    private void evalText(RESTResponse restResponse, Object expectedOutput) {
        Allure.step("Evaluating with ExactMatcher, Actual : " + restResponse.getResponse().getBody() + " Expected : " + expectedOutput);
        Assert.assertEquals(restResponse.getResponse().getBody(), expectedOutput, " Missmatch of Expected vs Actual ");
    }
