...
public class RestelRunner {
     ...
     */
    public void run() {
        suiteExecutor.executeTest(testManager.getTestSuites().values().stream().filter(suite -> CollectionUtils.isEmpty(suite.getParentSuites()) && suite.isSuiteEnable())
                .map(this::createSuite).collect(Collectors.toList()));
    }
    ...
    private XmlSuite createSuite(RestelSuite suite) {
        ...
        if (!CollectionUtils.isEmpty(suite.getDependsOn())) {
            suite.getDependsOn().forEach(dependentSuite -> {
        ...
        parentSuite.setTests(
                ...
                new ArrayList<>(getTestList(parentSuite, testManager.getExecutionDefinitions().stream().filter(RestelExecutionGroup::isTestExecutionEnable).filter(e ->
                        e.getTestSuiteName().equals(suite.getSuiteName())).filter(exec -> CollectionUtils.isEmpty(exec.getParentExecutions())).collect(Collectors.toList()), tests)));
     ...
     *
     * @param parentSuite The suite instance to which the created {@link XmlTest}
     *                    instances will fall under.
     * @param restelExec  The {@link RestelTestMethod}s that will be translated.
     * @return List of {@link XmlTest} that has been translated.
     */
    private static Set<XmlTest> getTestList(XmlSuite parentSuite,
                                             List<RestelExecutionGroup> restelExec, Set<XmlTest> tests) {
        for (RestelExecutionGroup exec : restelExec) {
            if (exec.isTestExecutionEnable()) {
                ...
                if (!CollectionUtils.isEmpty(exec.getDependsOn())) {
                    ...
                    exec.getDependsOn().forEach(dep -> dependentTest.addAll(getTestList(parentSuite, exec.getDependsOn(), tests)));
    ...
    /**
     * A new {@link XmlClass} instance for the given test. For the standard Test
     * class that we have.
     *
