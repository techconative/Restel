package com.techconative.restel.core.parser.config;
...
/**
 * Master configuration for the excel parser.
 * Configuration includes:
 * 1) sheetMap: <sheetname, sheetconfig>.
 * sheetname: name of the sheet in excel
 * sheetconfig includes
 * a) sheettype: enum with values ROW, COLUMN
 * b) fieldMap: <fieldname, cellFunction>.
 * fieldname: name of field in the sheet
 * cellFunction: value fetch function to get cell value based on cell type
 * 2) beanClassNameFunction: function which returns corresponding bean class name string for input string. Eg. Input: "base_config" Output: "BaseConfig"
 * 3) BEAN_PACKAGE_PATH: package path string where dtos are located. Eg. "com.restel.parser.dto"
 */
public class ParserConfig {

    Map<String, SheetConfig> sheetMap = new HashMap<>();

    Map<String, String> sheetNameToClassMap = Map.of(ParserEnums.TEST_DEFINITIONS.toString(), ParserEnums.TEST_DEFINITIONS.getValue(),
            ParserEnums.TEST_SUITE_EXECUTION.toString(), ParserEnums.TEST_SUITE_EXECUTION.getValue(),
            ParserEnums.TEST_SUITES.toString(), ParserEnums.TEST_SUITES.getValue(),
            ParserEnums.BASE_CONFIG.toString(), ParserEnums.BASE_CONFIG.getValue());

    UnaryOperator<String> beanClassNameFunction = sheetName -> sheetNameToClassMap.get(sheetName.toUpperCase());

    public static final String BEAN_PACKAGE_PATH = "com.techconative.restel.core.parser.dto";
    ...
    /**
     * SheetType enum
     * ROW: header field values are available in first cell of each row
     * COLUMN: header field values are available in first cell of each column
     */
