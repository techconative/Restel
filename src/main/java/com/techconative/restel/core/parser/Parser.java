...
import com.techconative.restel.core.parser.config.ParserConfig.SheetConfig;
import com.techconative.restel.core.parser.config.ParserConfig.SheetType;
...
/**
 * Parser type.
 * Parses the excel file based on ParserConfig and returns map of shee name and bean objects in the sheet
 */
@Slf4j
public class Parser {
    ...
    /**
     * Parse inputStream of xlsx file and returns map of sheet name and corresponding bean object(s) in the sheet
     * Sample run:
     * FileInputStream inputStream = new FileInputStream(new File("/home/restel/Sample_Suite_definition.xlsx"));
     * Parser parser = new Parser(ParserConfig.load());
     * Map<String, Object> beanObjectMap = parser.parse(inputStream);
     *
    ...
    public Map<String, Object> parse(InputStream inputStream) {
       ...
       */
            for (Map.Entry<String, SheetConfig> entry : this.parserConfig.getSheetMap().entrySet()) {
         ...
         */
                if (SheetType.ROW.equals(sheetConfig.getSheetType())) {
                    Object beanObject = rowTypeSheetParser(sheet, sheetConfig);
                    beanObjectMap.put(sheetName, beanObject);
                } else if (SheetType.COLUMN.equals(sheetConfig.getSheetType())) {
     ...
     */
    private Object rowTypeSheetParser(Sheet sheet, SheetConfig sheetConfig) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
     ...
     */
    private List<Object> columnTypeParser(Sheet sheet, SheetConfig sheetConfig) throws ClassNotFoundException, NoSuchMethodException, InstantiationException, IllegalAccessException, InvocationTargetException {
     ...
     */
    private Object populateBean(Sheet sheet, Map<String, Integer> fieldMap, SheetConfig sheetConfig,
                                Integer rowNum, Integer columnNum) throws InvocationTargetException, IllegalAccessException, ClassNotFoundException, NoSuchMethodException, InstantiationException {
        Class<?> beanClass = Class.forName(ParserConfig.BEAN_PACKAGE_PATH + "." +
        ...
        if (Objects.isNull(rowNum) && !Objects.isNull(columnNum)) {
            populateSheet(sheet, fieldMap, sheetConfig, columnNum, methods, beanObject, SheetType.ROW);
        }
        //SheetType = COLUMN
        else if (!Objects.isNull(rowNum) && Objects.isNull(columnNum)) {
            populateSheet(sheet, fieldMap, sheetConfig, rowNum, methods, beanObject, SheetType.COLUMN);
        } else {
            beanObject = null;
        }
        return beanObject;
    ...
    private void populateSheet(Sheet sheet, Map<String, Integer> fieldMap, SheetConfig sheetConfig,
                               Integer sheetTypeNum, Method[] methods, Object beanObject, SheetType sheetType) throws InvocationTargetException, IllegalAccessException {
        for (Map.Entry<String, Function<Cell, ?>> e : sheetConfig.getFieldMap().entrySet()) {
            String fieldName = e.getKey();
            if (fieldMap.containsKey(fieldName)) {
                Cell cell = sheetType == SheetType.ROW ? sheet.getRow(fieldMap.get(fieldName)).getCell(sheetTypeNum) :
                        sheet.getRow(sheetTypeNum).getCell(fieldMap.get(fieldName));
                if (!isBlank(cell)) {
                    Optional<Method> methodOptional = Arrays.stream(methods)
                            .filter(m -> Functions.SETTER.apply(fieldName).equals(m.getName())).findFirst();
