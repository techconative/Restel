...
/**
 * Implementation responsible for invoking a request, taking care of the pre-processing and
 * post-processing of the requests
 */
public class RequestManager {
  ...
  /**
   * Makes a API call with the given parameters. Execute pre and post middlewares before and after
   * making the API calls, in the given order.
   *
   ...
   * @param postRequestMiddlewares The list of {@link ResponseMiddleware} to be executed in order.
   * @return {@link RESTResponse} instance with the response for the given call.
   */
  public RESTResponse makeCall(
    ...
    // given order
    RESTRequest processedRequest = performAll(preRequestMiddlewares, request);
    ...
    // given order
    return performAll(postRequestMiddlewares, response);
  }

  /**
   * Applies all the middleware operations in the given order on the given data that is to be
   ...
   * @return The middleware data after all the middleware operations being applied in order.
   */
  private <T> T performAll(List<? extends HttpMiddleware<T>> middlewares, T middlewareData) {
