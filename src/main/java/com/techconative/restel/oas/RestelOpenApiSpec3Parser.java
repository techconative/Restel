...
public class RestelOpenApiSpec3Parser {
  ...
  private TestDefinitions defineTestDefinition(
    ...
    def.setRequestQueryParams(
        getParams(
                parser.getPaths().get(path).getParameters(),
  ...
  /**
   * returns an ObjectNode of response headers from APIResponse with status code 200 first .If not
   * present search for default response, else takes the first element({@link ApiResponse}) from
   * {@link ApiResponses}.
   *
  ...
  /**
   * returns an ObjectNode from APIResponse with status code 200 first .If not present search for
   * default response, else takes the first element({@link ApiResponse}) from {@link ApiResponses}.
   *
   * @param responses {@link ApiResponses}
  ...
  private Object getRequestBody(RequestBody requestBody) {
    if (StringUtils.isNotEmpty(requestBody.get$ref())) {
      return getModelFromComponent(
          getRefName(requestBody.get$ref()), getRefType(requestBody.get$ref()));
   ...
   * @return get JsonNode of the Content of MediaType: application/json first if present ,or lese
   *     return the first element from the Content Map.
   */
   ...
   */
  private ObjectNode getParams(
      List<Parameter> endpointParameters, List<Parameter> opParameters, String type) {
  ...
  private void addParametersByType(List<Parameter> parameters, String type, ObjectNode paramsNode) {
    parameters.parallelStream()
        .filter(parameter -> StringUtils.containsIgnoreCase(parameter.getIn(), type))
        .forEach(
            par -> {
              if (!Objects.isNull(par.getSchema())
                  && StringUtils.isNotBlank(par.getSchema().getType())) {
                paramsNode.putPOJO(par.getName(), getModel(par.getSchema()));
              } else if (validRef(par.get$ref())) {
                paramsNode.putPOJO(
                    par.getName(),
                    getModelFromComponent(getRefName(par.get$ref()), getRefType(par.get$ref())));
              }
            });
  }
   ...
   * @return If index= 0 the ref (from example '#/components/schema/Pet') is returned, else if
   *     index=1 then Type ( from example 'schema') is returned,else if index=2 name (from example
   *     'Pet') is returned.For order index values will return null.
   */
  private String parseRef(String ref, int index) {
    try {
      ...
      log.warn(
          "invalid index: {} should be one of [0,1,2] for Schema reference format:{} ", index, ref);
    }
    return null;
  }
   ...
   *
   * @param schemaName schema name from the swagger components
   *     format:#/components/<schemaType>/<schemaName>
   * @param schemaType schema Type from the swagger components
   *     format:#/components/<schemaType>/<schemaName>
  ...
  private Object getModelFromComponent(String schemaName, String schemaType) {
    ...
    if (!Objects.isNull(components)) {
      // get RequestBodies
      if (StringUtils.containsIgnoreCase(schemaType, Constants.REQUEST_BODIES)
          && !Objects.isNull(components.getRequestBodies())
      ...
      // get schema
      else if (StringUtils.containsIgnoreCase(schemaType, Constants.SCHEMA)
          && !Objects.isNull(components.getSchemas().get(schemaName))) {
        return getModel(components.getSchemas().get(schemaName));
      } // get Parameter
      else if (StringUtils.containsIgnoreCase(schemaType, Constants.PARAMETER)
          && !Objects.isNull(components.getParameters().get(schemaName))) {
      ...
      else if (StringUtils.containsIgnoreCase(schemaType, Constants.HEADER)
          && !Objects.isNull(components.getHeaders().get(schemaName))) {
        return getHeader(components.getHeaders().get(schemaName));
      } else if (StringUtils.containsIgnoreCase(schemaType, Constants.RESPONSE)
          && !Objects.isNull(components.getResponses().get(schemaName))) {
        return getResponseBody(components.getResponses().get(schemaName));
      } else {
        log.warn(
            "Missed adding model for the component with schemaName:{} ,schemaType:{} ",
            schemaName,
            schemaName);
      }
    }
  ...
  /**
   * @param ref reference String, should be of format:#/components/<schemaType>/<schemaName> . eg:
   ...
   * @return return a JsonNode from the schema. if schema is a primitive type return string value of
   *     the schema type (like 'string','integer',etc.,)
   */
  private Object getModel(Schema<?> schema) {
    if (schema instanceof ArraySchema) {
      return ObjectMapperUtils.getMapper()
          .createArrayNode()
          .addPOJO(getModel(((ArraySchema) schema).getItems()));
  ...
  /**
   * If schema has oneOf or anyOf, will construct the JsonNode with the first element. If the schema
   * has allOf, will combine the attributes from each of the elements and returns JsonNode.
   *
  ...
  private Object getComposedModel(ComposedSchema schema) {
    ...
    try {
      if (CollectionUtils.isNotEmpty(schema.getAllOf())) {
        List<Object> data =
            schema.getAllOf().parallelStream()
                .map(this::getModel)
        ...
        Optional<Object> value =
            schema.getAnyOf().parallelStream()
                .map(this::getModel)
                .filter(a -> !Objects.isNull(a))
                .findFirst();
        if (value.isPresent()) {
        ...
        Optional<Object> value =
            schema.getOneOf().parallelStream()
                .map(this::getModel)
                .filter(a -> !Objects.isNull(a))
                .findFirst();
        if (value.isPresent()) {
    ...
    if (MapUtils.isNotEmpty(schema.getProperties())) {
      ObjectNode node =
          n instanceof ObjectNode
              ? (ObjectNode) n
              : ObjectMapperUtils.getMapper().createObjectNode();
